package springmvc.board.global.jwt.service;
...
class JwtServiceImplTest {
    ...
    @Test
    public void createRefreshToken_발급() throws Exception {
        //given,when
    ...
    public void extractAccessToken_AccessToken_추출() throws Exception {
        ...
        //when
        String extractAccessToken = jwtService.extractAccessToken(httpServletRequest).orElseThrow(() -> new Exception("토근이 없습니다."));


        //then
        assertThat(extractAccessToken).isEqualTo(accessToken);
        assertThat(getVerify(String.valueOf(extractAccessToken)).getClaim(USERNAME_CLAIM).asString()).isEqualTo(username);
    }
    ...
    public void extractRefreshToken_RefreshToken_추출() throws Exception {
        ...
        //when
        String extractRefreshToken = jwtService.extractRefreshToken(httpServletRequest).orElseThrow(() -> new Exception("토근이 없습니다."));
    ...
    public void extractUsername_Username_추출() throws Exception {
        ...
        //when
        String extractUsername = jwtService.extractUsername(requestAccessToken).orElseThrow(() -> new Exception("토근이 없습니다."));
